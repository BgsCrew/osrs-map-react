...
jest.spyOn(coordinateUtils, 'osrsToLatLng').mockImplementation((map, x, y) => ({
  lat: y / 100,
  lng: x / 100
} as any));
...
describe('useOSRSMap Hook', () => {
  ...
  test('should initialize with null current position', () => {
    const { result } = renderHook(() => useOSRSMap({
      map: null,
      plane: 0
    }));
  ...
  test('should clean up mouse move listener on unmount', () => {
    const { unmount } = renderHook(() => useOSRSMap({
      map: mockMap as any,
      plane: 0
    }));
